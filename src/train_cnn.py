...
from src.models.cnn import KimCNN
...
def run(epochs=8, batch_size=32, lr=1e-3, emb_dim=200, n_filters=100, max_len=64, seed=42):
    ...
    model = KimCNN(len(vocab), emb_dim, num_classes=3, n_filters=n_filters, pad_idx=0).to(DEVICE)
    ...
    f1 = macro_f1(gold, preds)
    rep = full_report(gold, preds, LABEL_ORDER)
    cm  = confmat(gold, preds, LABEL_ORDER)

    os.makedirs("artifacts/cnn", exist_ok=True)
    torch.save({"state_dict": best_state, "vocab": vocab}, "artifacts/cnn/kimcnn.pt")
    plot_confusion(cm, LABEL_ORDER, "artifacts/cnn/kimcnn_cm.png")
    save_json({"test_macro_f1": float(f1), "report": rep, "cm": cm.tolist()}, "artifacts/cnn/kimcnn_results.json")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--n_filters", type=int, default=100)
